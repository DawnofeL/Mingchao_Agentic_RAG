...
完整交互式报告见 [`output/`](output/) 下各 HTML 文件，所有题目和逐题打分均可查阅。



> **Agentic vs Vector：100 题综合对比**

同一份 100 道题（50 道人物、50 道时间线），300 个 keypoint，两种模式相同 judge 打分：

|             | Keypoint 召回率      | 人物类    | 时间线类  | 完全无法回答 |
| ----------- | -------------------- | --------- | --------- | ------------ |
...
语料只覆盖前三卷，题库专挑后半部分内容检测系统有没有在工具返回空结果时用先验知识编造答案。测试结果为**100 道题，recall rate 100%，zero hallucination。**



> **Vector 模式 chunk 检索：140 题基础能力**

|                | Recall@1  | Recall@10 | NDCG@10   |
| -------------- | --------- | --------- | --------- |
| RRF            | 0.643     | 0.929     | 0.793     |
| RRF + Reranker | **0.757** | **0.957** | **0.855** |

Agentic 模式实际使用 top-10 结果，Recall@10 = 0.957 意味着绝大多数题目正确 chunk 几乎总能进入喂给 LLM 的材料里。
