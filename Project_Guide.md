...
编排器是按轮次推进的。每一轮先从清单里挑出“依赖都已经满足”的任务，一起并发执行，跑完把答案存进结果池，再开下一轮，直到所有任务都有了着落。
...
每个任务的依赖 `depends` 都是空的，所以可以**一次性丢进线程池并发跑**。

这样安排是为了把能省的等待全省掉，三件事本来就没有先后关系，没必要排队一个个来。顺带一提，万一清单里的依赖兜成了环，A 等 B、B 又等 A，那么某一轮会一个能跑的都挑不出来，编排器会判定卡死直接停，不会空转下去。
...
前面几节其实已经把这件事的具体做法讲过了，只是分散在意图识别、检索工具、编排器三处，没点名这其实是同一套设计思路。这里把它们串起来讲一遍，顺便说清楚我对这个概念的理解。



### i. 我理解的 Harness Engineering

LLM 这类东西最大的麻烦不是它不够聪明，而是它的输出本质上是概率采样出来的，同一句 prompt 跑十次，格式、用词、要不要填某个参数，都有概率跑偏。如果整套系统的可靠性完全押在“模型这次会不会听话”上，那系统稳定性跟开盲盒没什么区别。

Harness Engineering 说的就是，**别让 LLM 自己对自己的输出负责，把“输出对不对”这件事交给确定性的代码去把关**。LLM 只负责“理解和决策”这种只有它能干的活，剩下格式合不合法、参数取值在不在允许范围内、出错了要不要重试，这些能写成规则的部分，全部用代码兜底。说白了就是给这个不确定的核心套一层确定性的壳子，让系统的可靠性建立在代码上，而不是建立在“这次 LLM 心情好不好”上。

这两年这个词被讨论得多，是因为大家发现 agent 能不能真正用在生产环境，拼到最后拼的不是模型参数量，而是这层壳子打得有多扎实。
...
**意图识别节点（QU）**：见 [5.v 代码层校验与重试](#v-代码层校验与重试)，[`query_understanding.py`](rag/graph/nodes/query_understanding.py) 里 LLM 用 JSON 模式输出查询计划，代码逐条校验 task_id 编号、query_kind 枚举值、depends_on 引用合法性，校验不过就把错误描述喂回去让 LLM 重新生成，最多重试两次。这是这个项目里把“自我修正”做得最完整的一处。

**知识图谱检索工具的参数枚举**：见 [6.i 人物检索工具](#i-人物检索工具) 和 [6.ii 时间线检索工具](#ii-时间线检索工具)，`era_filter`、`primary_filter`、`era` 这类参数只能取固定枚举值。LangChain 的 `@tool` 类型签名只能约束参数是不是 `list[str]`，约束不了字符串内容是不是在枚举表里，这一层只能靠 docstring 文字嘱咐 LLM，不是强约束。真正的硬校验放在数据层，[`people_store.py`](rag/retrieval/people_store.py) 里的 `_Validate_Era_Filter` / `_Validate_Primary_Filter`，以及 [`timeline_store.py`](rag/retrieval/timeline_store.py) 里的 era 校验，对不在枚举表里的取值统一剔除并打日志报警：
...
这一步顺手还揪出一个真实 bug，`timeline_store.py` 原来的逻辑是只要 `era` 列表里全是非法值，直接 `return []`，连 `year`、`participants` 这些本来合法的条件也被一起拖空，跟它自己的文档注释"非法枚举值自动忽略"自相矛盾。改成了全部非法时退化成"跳过 era 过滤"，不影响其它条件，这才对得上文档原本的承诺。

**引用锚点的运行时校验**：[6.vii 召回溯源设计](#vii-召回溯源设计) 里讲过，答案里的 `[people_id=N]` / `[event_id=N]` / `[chunk_id=N]` 这类锚点之前完全是靠 skill 文本里"必须标 id、严禁编造"这句话管着，没有任何代码会去核对这个 id 是不是真的来自这一轮检索到的证据。新增了 [`citation_check.py`](rag/graph/citation_check.py)，在 [`route_people.py`](rag/graph/nodes/route_people.py)、[`route_timeline.py`](rag/graph/nodes/route_timeline.py)、`Synthesize_Answer`（[`route_task.py`](rag/graph/nodes/route_task.py)）、`_Synthesize_Final_Answer`（[`orchestrator.py`](rag/graph/nodes/orchestrator.py)）这条链路上的每一个文本输出点，都先把答案里的 id 抠出来，跟当前真实证据池（工具结果 / chunk 列表 / 上游子任务答案）做一次比对，对不上就打日志报警，直接把答案换成"根据现有资料，无法回答此部分"，不让这条答案流出去：
...
这一步把"溯源设计""零幻觉"这两个说法从单纯靠 prompt 约束，往前推到了有代码兜底的程度，是目前 Harness Engineering 这条线里覆盖范围最大的一次补强。

**Orchestrator 的解引用**：[`orchestrator.py`](rag/graph/nodes/orchestrator.py) 的 `_Parse_Resolved` 会先整体解析 JSON，失败了再用正则把 `{...}` 抠出来重试一次解析。

> [!TIP]
>
> **这里要老实说清楚一处不够严谨的地方。** `_Resolve_References` 的解析失败处理和意图识别不是同一种力度，意图识别失败了会把错误喂给 LLM 重新生成，解析失败了则是直接放弃这条任务，标记成阻塞，走兜底文案，并不会打回去让 LLM 重新输出。两种做法都属于 Harness Engineering 的范畴（都是用代码兜底，不让 LLM 输出失控），但严谨程度不一样，写文档不能含糊带过，更不能说成"每一步都重试到合法"。



### iii. 没做到的部分

Harness Engineering 完整版本里通常还包括系统化的可观测性，比如给每次 LLM 调用打 trace id、记录延迟和重试次数，方便事后排查哪一步出了问题。这个项目目前全靠 `print()` 打日志，没有结构化的调用记录，这一块暂时没做。
